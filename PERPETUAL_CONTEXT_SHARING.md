...
> **TL;DR：Cache 是永续上下文能够成立的经济基石。** 在 Claude 订阅制的实测中，命中缓存的历史前缀不会重复消耗套餐额度，每轮主要为新增输入和模型输出付费。对于 Claude Code 这类会在工具调用间反复读取长上下文的 Agent，这让“持续保留工作现场”从昂贵的重复计算变成可长期复用的默认选择。具体额度、推导过程及适用边界见 [《Suspiciously precise floats, or, how I got Claude’s real limits》](https://she-llac.com/claude-limits)；其中数字来自作者 2026 年 1 月的非官方逆向与实测，可能随 Anthropic 的策略调整而变化。
...
> 模型训练给了 Agent 天赋。
> Context 给了 Agent 短期记忆。
> Memory 和 Tools 给了 Agent 成长空间。
