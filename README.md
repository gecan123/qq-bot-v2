...
1. 收到群消息
2. 解析消息、处理媒体、写入数据库
3. 如果消息包含 `@SELF_NUMBER`，投递到独立 conversation queue
4. scheduler 按群维度做 30 秒聚合窗口
5. 同群串行执行 worker，不同群可并行
6. worker 读取数据库消息、生成正式回复，再通过发送层回消息
...
- 单轮最多处理 2 个 sender 线程，剩余内容自动进入下一轮
- 默认尽量一条发完，不发 ack
...
│   ├── bot/              # NapCat 接入、消息处理
│   ├── config/           # 环境变量和配置
│   ├── conversation/     # 异步@会话调度、dispatcher、worker
