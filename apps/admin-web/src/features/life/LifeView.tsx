...
export function LifeView({ snapshot, isRefreshing, refreshFailed }: { snapshot: LifeSnapshot; isRefreshing: boolean; refreshFailed: boolean }) {
  ...
  return <>
    <PageHeader title="生命状态" description="Goal 是唯一持久执行主线；Agenda、Schedule 与 Background Task 按各自真实存储读取。" generatedAt={snapshot.generatedAt} isRefreshing={isRefreshing} refreshFailed={refreshFailed}/>
    <StatGrid>
