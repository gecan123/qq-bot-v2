...
describe('OverviewView', () => {
  test('renders the read-only operational snapshot', () => {
    ...
    assert.ok(screen.getByText('正在打开网页'))
  ...
  test('renders explicit empty runtime state', () => {
    render(
      <OverviewView
        snapshot={{
          ...snapshot,
    ...
    assert.ok(screen.getByText('无法确认实时步骤'))
    assert.ok(screen.getAllByText('实时状态不可用').length >= 1)
