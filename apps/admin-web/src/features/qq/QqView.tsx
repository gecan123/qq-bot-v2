...
export function QqView({ snapshot, isRefreshing, refreshFailed }: { snapshot: QqSnapshot; isRefreshing: boolean; refreshFailed: boolean }) {
  ...
  const groups = useMemo(() => snapshot.groups.filter(group => !normalizedSearch || `${group.name} ${group.groupId}`.toLocaleLowerCase().includes(normalizedSearch)), [normalizedSearch, snapshot.groups])
  ...
  return <>
    ...
    <div className="filter-toolbar mt-4"><SearchInput value={search} onChange={setSearch} label="搜索 QQ" placeholder="搜索群名、群号、发送者或消息"/><span className="filter-count">{groups.length} / {snapshot.groups.length} 个群 · {messages.length} / {snapshot.messages.length} 条最近消息</span></div>
    <Panel className="mt-4" title="群聊" description="按最后活跃时间排序；进入后只显示该群数据。">{groups.length ? <div className="group-grid">{groups.map(group => <Link key={group.groupId} to="/qq/group/$groupId" params={{ groupId: group.groupId }} className="group-card"><span className="group-card-icon"><MessagesSquare size={17}/></span><span className="min-w-0 flex-1"><strong>{group.name}</strong><small>群 {group.groupId} · {formatCount(group.messageCount)} 条<br/>最后活跃 {formatTimestamp(group.lastAt)}</small></span><ArrowUpRight size={14}/></Link>)}</div> : <EmptyState>没有匹配的群聊</EmptyState>}</Panel>
