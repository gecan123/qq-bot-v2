...
- bot 在 QQ 与飞书所有允许来源间共享一个串行 `AgentContext`。异步来源通常追加不含正文的 `notification` envelope；`priority` 表示重要性，`delivery=interrupt|next_round|passive` 独立决定披露节奏，`open.tool/open.args` 指向来源自己的按需读取入口。两个平台的正文先写 `messages` / `media`：私聊、结构化 @bot、编辑和撤回可以形成 attention；普通群消息默认由 `inbox list/read` 被动、有界读取，QQ 的 selective/active 群还可以聚合为 normal+passive。provider-confirmed 群外发会给该 mailbox 建立 30 分钟 engagement lease；lease 内多个群的新消息，以及群内结构化 @，可以在同一 debounce 批次以有界 `conversation_deltas` 直接追加。直接披露和对应 `inboxReadCursors` 必须同事务提交；超出 12,000 字符上限的批次保持未读并退回普通 inbox notification。
...
- 正常在线期间聚合出的 mailbox notification 无论数量多少，都从第一条未读事实开始分页，不因群聊刷屏自动跳到最近一页。只有进程启动恢复产生的 `mailbox_backlog` 可以把超过阈值的普通群消息默认打开到最近 50 条；私聊，以及缺席范围内包含结构化 @ 或消息生命周期事件的群 backlog，必须从第一条未读开始完整披露。
...
- conversation focus 只决定下一次外发 target，不是唯一读取订阅。多个群可以同时持有 engagement lease；成功群外发创建或刷新 lease，lease 内直接披露的新消息继续刷新它，过期后恢复 metadata notification。该 lease 保存在已有 mailbox continuity JSON 中，不进入 prompt/system 前缀；`conversation_deltas` 只追加在 canonical tail，不改写历史前缀。
...
- 主动休息只由主 Agent 显式调用 `rest`，默认请求 30 分钟、范围 10..360；等待发生在该工具执行内部，不读取或改写 canonical projection。工具只在当前进程保存最近三小时的实际休息区间：按 Asia/Singapore，白天 06:00..24:00 最多累计 60 分钟，夜间 00:00..06:00 最多 360 分钟；本次批准时长取请求、剩余额度和下一昼夜边界的最小值，夜间休息最晚在 06:00 自然结束并交还控制，注意事件打断只记录实际经过时间。自然结束后 Runtime append 一次 `runtime_autonomy_tick` 并在该探索阶段拒绝再次 `rest`，直到一次工具行动获得真实进展；被注意事件打断则优先处理该事件。休息额度和探索门控均不进入 runtime singleton、不跨重启持久化，也不根据工具次数、工作量或所谓精力判断资格。
