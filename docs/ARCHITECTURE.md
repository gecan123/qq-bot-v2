...
- 连续轮次和空闲循环有进程内有界保护；它们不进入 ledger。工具用 `outcome.progress` 报告是否获得新事实或改变状态，用 `retryClass=immediate|after_event|backoff|terminal` 提供调度提示；可立即纠正的失败仍受三轮上限保护。`curiosity_tick` 只保留为人工调试入口。
