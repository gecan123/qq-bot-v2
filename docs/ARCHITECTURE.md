...
- `pause action=rest` 是短休息安全阀，时长 30–300 秒。首次请求使用 `confirmed=false`；Life Journal idle picker 若从 Agenda、近期 Journal 或愿望中找到未完方向，工具只把 `alternative_available` 写入 ledger 而不产生 pause effect。模型看过建议后仍想休息，才以 `confirmed=true` 再次调用。醒后计划只保留一个 `primaryDirection` 和一个不同的 `alternativeDirection`；等待消息、机械盯行情和泛化维护不是方向，未来时点检查应使用 `schedule`。真正计时结束后自动继续，私聊、`@bot`、后台任务完成和停止信号可提前打断。
