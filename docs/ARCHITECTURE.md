...
`apps/admin-web` 是独立的 TanStack Start Node 应用，不参与 bot 启动、ingress 或主 Agent 调度。当前只实现 Overview：展示 ledger head/count、runtime/focus、当前 Goal、最近 `agent.chat` token/cache 和 24 小时工具调用摘要。
