...
6. `src/agent/bot-loop-agent.ts` 是唯一 Runtime Host；它把 expected-head append/增量 projection 委托给 `LedgerCommitCoordinator`，把 threshold/overflow/behavioral reset 与 CAS 重算委托给 `CompactionCoordinator`，把 continue/wait/backoff/stop 决策委托给纯 `LoopPolicy`。事务成功后才推进内存 `AgentContext`。
...
- 自动压缩由动态 token threshold、provider context overflow 或自主行为重置触发。overflow 每轮最多强制 compact-and-retry 一次；三个轮换探索方向全部无进展时，behavioral reset 追加一条清洁事实交接并从 active projection 移除旧循环，永久 ledger 不删不改。主 Agent 和聊天控制面不提供手动 compaction。
...
- `rest` 是唯一的主动暂停工具。它在工具调用内部按明确的批准时长等待，记录真实理由和醒后检查点；私聊、@、后台任务完成、调度事件或停止信号会提前打断。工具用一个进程内三小时滚动窗口限制实际休息：Asia/Singapore 白天最多 60 分钟，夜间最多 360 分钟，并在 00:00 / 06:00 边界结束后重新评估；夜间请求即使更长也会在 06:00 自然结束并交还控制。请求会被剩余额度缩短，额度不足 10 分钟时明确拒绝并技术退避。休息自然结束后进入有界自主探索，此时工具列表保持稳定，但 Runtime 会拒绝再次 `rest`；只有获得真实新证据或改变可观察状态后才解除限制。休息区间与该探索门控都不进入 runtime singleton，也不跨重启持久化。
- Runtime 不维护自动休息顾问或持久 Goal。全新空 ledger 会先 append 一条稳定的自主启动消息并立即开始第一轮；事件等待最长 30 分钟，外部事件可随时提前唤醒，超时则 append 一条 canonical `runtime_autonomy_tick`，轮换检查未完成承诺、已有产物和有界好奇探索。自主探索期的心理记录、Notebook/Memory 自我维护、会话开关、外发总结、Schedule、Clock、Help 和 Skill 管理不计作方向进展；三个方向都连续失败时，现有 compaction seam 生成过滤掉重复自述的事实交接。轮换索引、失败方向计数与当前探索门控只保存在进程内，不新增队列、进程或第二个 Agent。
