...
| s05 TodoWrite | 有意不实现 | 当前连续执行状态由主循环和 tool result 直接承接；跨天过程用 Notebook，未来时点重新评估用 Schedule，不维护独立 Todo 或持久任务状态。 |
...
| s09 Memory | 已满足核心 | 主 Agent 的 `memory` 只暴露 `remember/recall/correct`；recall 做有界相关召回并返回 opaque ref，文件、revision、生命周期和整理都留在模块内部。Notebook 只保存跨天主题过程。 |
...
| s12 Task System | 有意不实现 | 当前连续行动由主循环和 tool result 承接，跨天过程用 Notebook，未来唤醒用 Schedule。没有多任务图、依赖、认领或 blockedBy DAG。 |
...
| s17 Autonomous Agents | 产品目标上已较强满足 | 主 Agent 在发送或完成其他动作后持续寻找下一件事；只有显式 `rest` 才主动暂停且可被注意事件打断，不设置普通 idle backoff、每日 token 预算或跨日限流。Schedule 负责未来时点重新评估。 |
...
| s20 Comprehensive | 单 Agent 产品骨架成熟 | 已有单循环 + 永续 context/replay + working projection + mailbox + deferred typed tools + 工具内权限边界 + recovery + compaction + durable typed background task/schedule + explainable memory + hooks + skill + 安全并行 + 自主循环。 |
...
4. s12 的多任务图/依赖、s15/s16 多 Agent team/protocol、s18 worktree isolation 只在 Luna 真正需要长期协作或自主改代码时引入；不提前增加第二主循环。
