...
| s17 Autonomous Agents | 产品目标上已较强满足 | 主 Agent 在发送或完成其他动作后持续寻找下一件事；只有显式 `rest` 才主动暂停且可被注意事件打断，不设置普通 idle backoff、每日 token 预算或跨日限流。显式 Life Journal/Agenda 保持生活连续性，active Goal 会在每轮和 compaction 后重注入为默认主线。 |
