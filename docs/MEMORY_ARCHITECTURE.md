...
- person/group 的 write、update 和 correction 必须引用真实存在的 `Message.id`；`messages` 仍是入站事实账本，QQ 号仍是实体主键，昵称/群名片只是 `qq_directory profile` 可披露的带来源观察值。`correct_entry` 用一次 revision-checked 原子写 supersede 旧 entry 并创建 replacement，避免先删后写窗口。
...
- 同一次 review 最多返回三个长期 Memory 候选；新条目只写入 recent tier，并进入现有 maintenance 队列。person/group 候选必须提供明确实体 ID 和本轮真实 `Message.id`。
