...
本项目的“记忆”不是一个统一数据库，也不等于所有持久化状态。当前目标模型分成三条主线：入站事实账本保存发生过什么，`AgentContext` 保存 LLM 已经看到什么，workspace side-data 保存以后可能按需取回什么。
...
- compaction 失败不会截断或覆盖旧 ledger；snapshot current 损坏时只从已验证 checkpoint 恢复，无法恢复就 fail closed。
...
| LLM ledger | `bot_agent_snapshot.context_snapshot` | 是 | Runtime Host 受控 append / compaction | 每轮自动使用 | 唯一持久 prompt history |
...
`activeToolCapabilities`、mailbox cursors、continuity 和 goal revision 随 snapshot 持久化，但属于运行控制状态，不属于 LLM 可见记忆。完整不变量见 `docs/AGENT_CONTEXT.md`。
...
flowchart LR
  ...
  F --> B["mailbox 元数据通知"]
  B --> C["AgentContext durable ledger"]
  F --> I["inbox / read_file"]
  I -->|"有界 tool result"| C
  C --> W["working-context projection"]
  W --> L["LLM"]
...
- `recall` 扫描 Markdown entry 后做确定性 lexical scoring：精确 ID/alias、短语、分词命中和稳定层级形成可解释分数；`search` 做文件级发现，`review` 只给重复/冲突候选。相同输入和文件内容必须得到相同排序。
...
- 异步 Life review 和 idle intention picker 会读取 Agenda；前者当前也可能更新它。

## 读取与披露

- `AgentContext` 每轮自动可见；其他层都不是常驻 prompt。
- 遇到具体人、群、旧话题、偏好或自己做过什么时，主 Agent 应先 `memory recall`。
...
当前采用**显式工具召回**：新消息先进入正常 Agent round；主 Agent 根据消息是否涉及具体人、当前群、旧话题、偏好、承诺或自己做过的事，决定是否调用 `memory recall`。recall 扫描当时的 Markdown，返回有界结果；这个 tool call 和 tool result 随即进入 `AgentContext`，后续 replay 直接使用已保存的结果，不重新扫描已经变化的 Markdown。
...
- 召回结果必须作为普通 ledger message/tool result append 并立即保存 snapshot，不做 request-time 隐藏注入。
...
- 只有 `AgentContext` 是 LLM ledger；不得从 memory、Notebook、Life Journal、Agenda、Goal 表或日志重建 transcript。
- `messages` 是入站事实账本，不是完整 prompt history。missed replay 只重新披露未处理事实。
- compaction 只改写 ledger prefix，并保持 assistant tool call 与 tool result 原子性。
...
每次用有效 runtime 状态覆盖当前 `bot_agent_snapshot` 前，旧的有效行会在同一事务中保存到 checkpoint 表，只保留最新 3 份。启动读取时先完整校验当前 snapshot；当前损坏时才按新到旧尝试 checkpoint，并明确记录恢复事件。当前与所有 checkpoint 都无效时 fail closed，不以 `messages`、side-data 或日志拼装替代历史。

checkpoint 只是同一 LLM ledger 的恢复副本，不是第二份 ledger，也不是普通 replay 路径。恢复后下一次保存可以用已验证的 runtime 状态替换损坏的 current，但不会把损坏行存成新 checkpoint。

## Reset 边界

`pnpm agent:reset-memory` 当前会同时删除：

- `bot_agent_snapshot`：LLM ledger 和披露游标；
...
- `src/agent/agent-context.ts`、`agent-context.types.ts`：LLM ledger。
- `src/agent/snapshot-repo.ts`：snapshot、mailbox cursors、continuity 和 goal revision 的原子持久化。
...
- `prisma/schema.prisma`：事实账本、snapshot、Goal 和观测表契约。
