...
- Memory、Notebook、Life Journal 和 Agenda 的人类可读 title/topic/content/事项正文以中文为叙述载体；命令、路径、URL、API 名、模型名和专有名词可以保留原文，但必须放在中文说明中。结构字段、ID 和固定英文分区名不翻译。
...
- 写入 schema 使用“至少包含中文叙述”的确定性最低门槛，prompt/tool description 负责完整表达规则；不使用容易误伤技术内容的中英文字符比例判断。
