...
- 为 docs links 和 prompt entry points 增加更多 `repo-check` 规则；Memory 架构的事实来源、checkpoint 和不可信数据边界已有基础检查。
...
- `agent:reset-memory` 实际会同时删除 snapshot、Goal、memory、Notebook 和 Life 状态；应改成更准确的 state reset 名称，或拆出可显式选择的 scopes。
