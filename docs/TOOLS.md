...
- 主动休息：`rest` 是唯一的主动暂停入口，参数为期望 `durationMinutes=10..360`（默认 30）、真实 `reason` 和醒后重新评估方向的 `resumeAction`。工具按实际经过时间维护进程内三小时滚动窗口：Asia/Singapore 白天 06:00..24:00 最多累计休息 60 分钟，夜间 00:00..06:00 最多 360 分钟；本次时长自动取请求、剩余额度和下一昼夜边界的最小值，因此夜间休息最晚在 06:00 自然结束并交还控制。注意事件提前打断时只计实际时间；额度不足 10 分钟返回 `rest_budget_exhausted` 并短暂技术退避。窗口不按工具次数或工作量解锁，不跨重启持久化；不要连续重试或创建 Schedule 等待额度。
...
- `inbox list` 只列出最近扫描窗口内 `latestRowId > lastReadRowId` 的待读来源；`read` 未显式传 `afterRowId` 时从持久已读 cursor 继续，并只推进到本次有界输出实际展示的最后一行。群读取必须显式指定监听白名单内的 groupId；私聊读取必须显式指定 peerId。read 结果用本地 `rowId` 作为 `send_message.reply_to.row_id`，`messageExternalId` 只用于披露平台关系；结构化 `media[].mediaId` 是入站媒体 handle，不能混作引用 ID。整体仍有行数和字符上限，并作为普通 tool result 进入 AgentContext。群文件上传 notice 会用稳定的负数 synthetic messageId 落入同一 mailbox，此时 `replyable=false`，只能 ambient 回复。provider-confirmed 群外发会为该 mailbox 创建或刷新 30 分钟 engagement lease；多个 engaged 群的新消息按 event debounce 聚合为 canonical tail 上的紧凑 `conversation_deltas`，保留本地 `rowId`、发送者、分钟时间、结构化 @、正文和 `mediaIds`，因此正常连续群聊不需要再次调用 inbox。直接披露与 `inboxReadCursors` 原子提交；单条正文最多 2,000 字符、整批最多 12,000 字符，超限批次不标已读并回退普通 inbox notification。结构化 @ 群消息即使尚未 engaged 也可直接披露。
...
- `notebook action=checkpoint|list|search|read` 把研究、阅读、市场观察、项目过程和其他主题的当前状态存到 `notebook/<kind>/YYYY-MM.md`。`checkpoint` 接收稳定单行 topic 和可独立恢复的完整当前状态：topic 不存在时创建，存在时保留稳定 ID 并原子替换，同时收敛所有月份内同 kind、同 topic 的旧重复记录；完全相同则返回 unchanged。list/search 只返回每个 topic 的最新状态，文件、revision 和去重留在模块内部。Notebook 维护本身不算主循环进展；稳定结论写 memory。
