...
- 当前计划：`todo`（当前进程内的短期多步计划，最多一个 `in_progress`）。`items=[]` 只清空已有计划；列表已为空时返回 `changed=false`，不要求再次调用。
...
- QQ 目录：`qq_directory`（分页列出/搜索 NapCat 当前全部好友；群目录只披露当前已加入且位于 `BOT_TARGET_GROUP_IDS` 的群；`profile` 按 QQ 号合并当前目录名和消息事实账本中观察到的历史群名片/昵称）。
...
- 激活状态保存在 `bot_agent_runtime_state.active_tool_capabilities`；QQ 当前会话保存在同一 runtime singleton 的 `qq_conversation_focus`。两者用于重启恢复运行控制状态，不是 LLM 可见事实，不写入 ledger message。focus 只由 `qq_conversation open/close` 改变，新 mailbox 不会自动切换它。
...
- `outcome.retryClass` 只给 Runtime Host 提供调度语义：`immediate` 允许有界纠错，`after_event` 等待外部事件，`backoff` 进入退避，`terminal` 不应原样重试。重复的 `inbox`、目录、Memory 和后台任务读取会返回 `progress=false`；后台任务仍在运行时返回 `after_event`。
...
- `qq_directory` 是只读目录。`list_friends` / `search_friends` 覆盖 NapCat 当前全部好友，因此这些结果都可作为 private `send_message` target；`list_groups` 只返回 NapCat 当前群列表与 `BOT_TARGET_GROUP_IDS` 的交集，不扩大群监听或发送授权。`profile` 以 QQ 号为主键，把当前好友 remark/nickname 与 `messages` 中同一 sender 的群名片、sender nickname、出现群和时间合并为带来源的 identity view；它不把昵称当权限或稳定事实。结果有条数上限和 offset 分页，不提供加删好友、加退群或群管理动作。
...
- `memory` 把长期记忆存到 `data/agent-workspace/memory/` 的 v1 Markdown 文件中；Markdown 是事实来源，当前没有 SQLite/FTS 或 embedding 索引。每条 entry 明确标记 `recent` 或 `stable`：普通 `write` 先进入 recent，完全相同且没有新增证据的同文件写入返回 `progress=false`；`promote_entry` 把单条线索提升为 stable，`compact` 用一条 stable 结论逻辑替代多条原记录，同时把旧 entry 标为 `superseded` 保留为可解析 provenance。person/group 的 `write`、`update_entry` 和 `correct_entry` 必须提供 `sourceMessageIds`，runtime 会验证这些 ID 确实存在于 `messages` 事实账本；`correct_entry` 在一次 revision-checked 原子写中 supersede 旧事实并创建带新证据的 replacement。已 superseded 内容不参与 search、recall、review 或 maintenance 阈值；仍被 `supersedes` 引用的 entry 不能直接 `delete_entry`。`topic write` 必须提供稳定 title，避免不同主题落入 `topics/topic.md`。`search` 只用于宽泛的文件发现；`recall` 扫描 entry 后按 ID/alias、短语、词项、tier 和 status 做确定性 lexical scoring，返回 sourceMessageIds、matchedTerms、分数与原因。person/group recall 必须带具体 QQ/群 `id`，只读取 `people/<id>.md` 或 `groups/<id>.md`，目标不存在返回空；self/topic recall 禁止 `id`，scope 和 id 都不传时保留全局探索，只有 id 而没有 scope 会被拒绝。当前聊天上下文不足且涉及旧事、偏好、稳定事实或经验时优先 recall；上下文已有足够且未冲突的信息时不要重复 recall。`review` 只读提出 duplicate/near_duplicate/possible_conflict，不自动改写。`read` 支持分页并返回 revision，`update_entry` / `correct_entry` / `delete_entry` / `promote_entry` / `compact` 要求最新 revision。四类长期状态的写入共用按资源键的单进程协调器；revision/CAS 继续负责拒绝 stale read-modify-write。旧格式不参与 list/search/read，下一次写入同一目标时直接以 v1 文件重建；原有 `delete` 继续用于永久删除明确文件。
