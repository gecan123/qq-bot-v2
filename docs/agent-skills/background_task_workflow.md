...
3. 如果任务还没完成，不要轮询它；立刻切换到另一件可执行的事。真实完成事件稍后会进入注意队列。
4. 收到后台任务完成事件后，用 `background_task action=get` 读取有界结果，再决定是否发送、收藏、继续处理或结束当前方向。

边界:

- 不要凭旧状态假设任务已完成。
- 不要为了轮询而连续空转，也不要因为一个后台任务未完成就停下整个 Agent；改做其他事。只有此刻真正想主动休息时才调用 `rest`。
