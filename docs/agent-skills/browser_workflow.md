...
需要真实网页交互、登录态、cookie、反爬验证、视觉布局判断或下载普通资源时使用本 skill。普通 URL 摘要优先用 `help action=activate capability=external_research` 后通过 `invoke tool=fetch_content args={...}` 读取，不需要打开浏览器。

入口:

- `browser` 是唯一浏览器内部工具，只有 `BOT_BROWSER_ENABLED=true` 且 `help action=activate capability=browser` 后可通过 `invoke tool=browser args={...}` 调用。
