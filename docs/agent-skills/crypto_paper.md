...
只有用户明确表达了币种、方向和数量时才能调用 `buy` / `sell`。市场研究、价格观察、投资观点、journal 中的计划以及 agent 自己的兴趣都不是交易授权；信息不完整时先询问，不要猜数量。
...
- 普通证券模拟仓继续使用 `workspace_bash moomoo trade/... --trd-env SIMULATE`。
