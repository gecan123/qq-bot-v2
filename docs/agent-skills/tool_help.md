...
- `workspace_bash command="help workspace"`: 私有工作区只读查看。
...
- `help action=activate capability=workspace_management`: 允许通过 `invoke` 调用 `workspace_file` 安全修改普通工作文件。
- `help action=activate capability=skill_management`: 允许通过 `invoke` 调用 `skill_editor` 创建、删除、校验和安装运行时 skill 草稿。
