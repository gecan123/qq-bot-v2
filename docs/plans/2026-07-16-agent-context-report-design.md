...
3. 默认 `pnpm agent:context` 输出 Claude Code 风格的终端概览；机器接口使用 `pnpm --silent agent:context -- --json` 暴露同一份结构化报告。
...
`pnpm --silent agent:context -- --json` 输出版本化 JSON，不混入日志或说明文字。`--silent` 是抑制 pnpm lifecycle banner、保持 stdout 可直接交给 `JSON.parse` 所必需；默认文本命令仍是 `pnpm agent:context`。JSON 是测试和未来 QQ/control adapter 的复用边界，但第一版不实现 adapter。
