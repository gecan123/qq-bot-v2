...
**Objective:** 让 Luna 在个人探索、创造和自然社交之间自主选择可验证的下一步，同时允许没有值得行动的方向时安静结束。

**Architecture:** 只修改启动时冻结的 `prompts/bot-system.md` 固定内核，不增加动态 prompt 注入，不改变 ledger、replay、mailbox 或工具契约。保留现有强制 `tool_choice`，其与无工具结束语义的冲突继续记录在技术债中。
...
重写 `[行动基线]`，使用正向、可执行的“形成候选 → 选择一步 → 获取证据 → 继续/replan/结束”循环；保留注意事件、安静结束、反机械忙碌和 token 预算边界。
...
Expected: 不包含 provider、runtime、ledger、mailbox、schema 或工具实现变更。
