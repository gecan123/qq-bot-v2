...
单轮 `generateReply` 链路已删除。`prompts/reply-instruction.md` 现作为 agent 的任务约束层使用；群聊人格基座放在 `prompts/default-persona.md`，由 `src/config/agent-profiles.ts` 读取，并允许 group 覆盖。
