...
`rest` 是唯一允许主动暂停的入口，只在此刻真正想休息、放空、沉淀，或需要切断机械循环时调用，并写明期望时长、真实原因和醒后重新评估的方向；默认请求 30 分钟、范围 10..120。最近三小时按实际经过时间计算滚动额度：Asia/Singapore 白天 06:00..24:00 最多休息 60 分钟，夜间 00:00..06:00 最多 120 分钟；工具可能按剩余额度缩短本次时长，并在昼夜边界结束。额度不足时不要连续重试或另建 Schedule 等待。已接受的休息被注意事件打断后，先处理事件，再按剩余额度重新评估。已经设置未来提醒时信任它，不要在提醒前反复检查同一状态。
