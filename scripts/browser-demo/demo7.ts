 ...
 *
 * 用法: pnpm tsx scripts/browser-demo/demo7.ts
 */
...
const SHOT_DIR = 'scripts/browser-demo/screenshots/demo7'
const MAX_STEPS = 500
...
const STRATEGY_REMINDER = `[每步必读 — 死亡是不可接受的失败]

⚠️ 死局 = 棋盘满 + 无相邻同值 = **之前所有努力归零**。死亡的负面价值远大于任何合并奖励。

每步必须按顺序问自己 3 件事 — 顺序不能颠倒：

1. **会死吗?** — 推演这步移动后, 棋盘还剩几个空格? 还有没有相邻同值? 如果空格 ≤ 1 且无任何相邻可合 = 已经在死亡边缘。如果这步把我推到死亡边缘 — 不论它能合多大数字、能守多牢角落、维持多漂亮蛇形 — **不选**。先排除致死方向。

2. **不会死的前提下, 哪步最有价值?** — 现在再考虑凑大数: 主战角落是哪个? 蛇形梯度怎样? 哪个方向能合并大数 / 制造未来合并 / 维持单调链? 长期目标永远是 1024 / 2048 / 4096, 不是局部 2+2=4。

3. **棋盘紧张时, 救场优先于守结构** — 空格 ≤ 3 时: 角落 / 梯度 / 蛇形都可以临时破。完美单调但无合并 = 必死。宁可丑陋活着, 也不要漂亮死掉。
...
async function callLLM(messages: ConvMsg[]): Promise<LlmTurn> {
  ...
  for (let attempt = 0; attempt < backoffs.length; attempt++) {
    if (attempt > 0) {
      console.log(`[demo7] retry attempt ${attempt + 1} after ${backoffs[attempt - 1]! / 1000}s (last=${lastStatus} ${lastErr.slice(0, 100)})`)
...
async function main(): Promise<void> {
  ...
  console.log(`[demo7] model=${MODEL} max_steps=${MAX_STEPS} accumulating history + per-step strategy inject`)
  console.log('[demo7] opening play2048.co')
  ...
  for (let step = 1; step <= MAX_STEPS; step++) {
    ...
    try {
      ...
      console.error(`[demo7] step ${step} LLM error:`, err)
      conversation.pop()
      endReason = 'llm_error'
      break
    }
    ...
    if (!key) {
      badCount++
      console.log(`[demo7] step ${String(step).padStart(3)}: BAD "${turn.direction}" raw="${turn.raw.slice(0, 80).replace(/\n/g, ' ')}" (${dt}ms ${shot.bytes.byteLength}B [${usageStr}])`)
    ...
    console.log(`[demo7] step ${String(step).padStart(3)}: ${turn.direction.padEnd(5)} (${dt}ms ${shot.bytes.byteLength}B [${usageStr}]) — ${turn.reason}`)
    ...
    if (await isGameOver(page)) {
      endReason = 'game_over'
      console.log(`[demo7] *** GAME OVER at step ${step} ***`)
      break
    }
  ...
  console.log('')
  console.log(`[demo7] ====== SUMMARY ======`)
  console.log(`[demo7] end_reason: ${endReason}`)
  console.log(`[demo7] total steps: ${log.length} (${okSteps} good, ${badCount} bad)`)
  console.log(`[demo7] elapsed: ${elapsedMin} min, avg ${avgLatency}ms/step`)
  console.log(`[demo7] tokens: input total ${totalIn}, output total ${totalOut}, cache_read total ${totalCacheRead}`)
  console.log(`[demo7] artifacts: ${SHOT_DIR}/`)

  await browser.close()
}

main().catch((err: unknown) => {
  console.error('[demo7] error:', err)
  process.exit(1)
})
