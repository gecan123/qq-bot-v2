...
async function main(): Promise<void> {
  ...
  const result = await canonicalizeSelfTopicMemory({ rootDir, apply })
...
async function assertBotStopped(projectRoot: string): Promise<void> {
  ...
  throw new Error(`bot is still running (pid=${pid}); stop it before canonicalizing memory files`)
}
...
async function assertNoBotProcess(projectRoot: string): Promise<void> {
  ...
  if (matches.length > 0) {
    throw new Error(`bot process still exists without a live pidfile; stop it before canonicalizing memory files:\n${matches.join('\n')}`)
  }
}
