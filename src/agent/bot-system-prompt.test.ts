...
describe('buildBotSystemPrompt', () => {
  test('keeps chat constraints and style details out of the resident system prompt', () => {
    ...
    assert.match(prompt, /help.*activate.*qq.*qq_conversation.*open.*invoke.*send_message/s)
    assert.match(prompt, /send_message.*message.*reply_to/s)
    assert.match(prompt, /CHAT_CONTEXT_UNAVAILABLE.*CHAT_CONTEXT_STALE.*重新打开/s)
    assert.doesNotMatch(prompt, /send_message\.replyToMessageId|send_message mode=ambient/)
