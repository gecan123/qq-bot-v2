...
describe('buildBotSystemPrompt', () => {
  test('keeps chat constraints and style details out of the resident system prompt', () => {
    ...
    assert.match(prompt, /反复只用同一类熟悉工具.*近期少用的低风险工具.*固定循环/s)
    assert.match(prompt, /现成图片.*collect_sticker.*inspect_media.*有文件.*read_file.*跨轮.*todo \/ notebook \/ goal.*skill.*finance \/ openbb_cli.*generate_image/s)
    assert.match(prompt, /不是工具覆盖率任务.*不要硬调用工具.*凑次数.*有成本\/副作用/s)
    ...
    assert.match(prompt, /alternative_available.*confirmed=true/s)
    assert.match(prompt, /时间晚、owner 不在线、群聊与你无关或刚完成一件事.*不是休息的充分理由/s)
    assert.match(prompt, /primaryDirection.*alternativeDirection.*不制造六项菜单/s)
