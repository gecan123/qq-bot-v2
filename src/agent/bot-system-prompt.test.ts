...
describe('buildBotSystemPrompt', () => {
  ...
  test('keeps the stable personality, I/O model, and progressive-disclosure entries', () => {
    ...
    assert.match(prompt, /conversation_deltas.*多群正文\/rowId.*勿调 inbox/s)
    ...
    assert.match(prompt, /reply_to.*row_id.*inbox\/delta.*rowId.*mentionedSelf=true.*expect=mentioned_self/s)
