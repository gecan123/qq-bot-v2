...
export function renderBotEvent(event: BotEvent): string | null {
  ...
  if (event.type === 'napcat_message') {
    ...
    return `[群:${groupLabel} | ${event.senderNickname}(QQ:${event.senderId})${mentionTag}] ${event.renderedText}`
  }

  if (event.type === 'napcat_private_message') {
    return `[私聊 | ${event.senderNickname}(QQ:${event.senderId})] ${event.renderedText}`
  }
