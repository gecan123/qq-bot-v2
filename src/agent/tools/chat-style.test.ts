...
describe('chat_style tool', () => {
  test('global scope returns style guide sections', async () => {
    ...
    assert.match(base.content as string, /有自己的新增才发.*一条消息只做一个动作/s)
    ...
    assert.match(antiPatterns.content as string, /把一瞬间反应整理成完整说明/)
    assert.match(antiPatterns.content as string, /纠正或复述.*解释.*总结或退场/s)
