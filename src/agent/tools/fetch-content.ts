...
const redditListArgsSchema = z.object({
...
const redditPostArgsSchema = z.object({
...
const argsSchema = z.discriminatedUnion('action', [
  ...
  imageUrlArgsSchema,
  qqAvatarArgsSchema,
  redditListArgsSchema,
  redditPostArgsSchema,
])
const externalResearchArgsSchema = z.discriminatedUnion('action', [
  urlArgsSchema,
  redditListArgsSchema,
  redditPostArgsSchema,
])
const mediaFetchArgsSchema = z.discriminatedUnion('action', [
  imageUrlArgsSchema,
  qqAvatarArgsSchema,
])

type Args = z.infer<typeof argsSchema>
export type FetchContentScope = 'all' | 'external_research' | 'media_fetch'
...
export function createFetchContentTool(deps: FetchContentDeps = {}): Tool<Args> {
  ...
  const schema = scope === 'external_research'
    ? externalResearchArgsSchema
    : scope === 'media_fetch'
      ? mediaFetchArgsSchema
      : argsSchema

  return {
    name: 'fetch_content',
    description: (scope === 'external_research'
      ? [
          '获取外部研究内容, 按 action 显式选择普通网页或 Reddit.',
          'action=url: 抓普通网页或文本并返回中文摘要; reddit 帖子请用 action=reddit_post.',
          '较慢或可与其他工作并行的网页抓取可传 background=true, 完成后用 background_task get 取结果.',
          'action=reddit_list: 刷受限 subreddit 列表; action=reddit_post: 深读单帖.',
        ]
      : scope === 'media_fetch'
        ? [
            '获取可发送、编辑或收藏的外部图片.',
            'action=image_url: 下载图片 URL, 返回 ephemeralRef.',
            'action=qq_avatar: 通过 QQ 号获取头像, 返回 ephemeralRef 和图片预览.',
          ]
        : [
    ...
    async execute(rawArgs, ctx) {
      const args = schema.parse(rawArgs) as Args
      if (args.action === 'url') {
...
export function fetchContentScopeAccepts(
  scope: Exclude<FetchContentScope, 'all'>,
  args: Record<string, unknown>,
): boolean {
  const action = args.action
  if (scope === 'external_research') {
    return action === 'url' || action === 'reddit_list' || action === 'reddit_post'
  }
  return action === 'image_url' || action === 'qq_avatar'
}
