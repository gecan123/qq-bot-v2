...
describe('wait tool — idle race', () => {
  ...
  test('idle timeout fires first → returns idle hint, does NOT enqueue wake', async () => {
    ...
    assert.match((result.content as string), /先挑一件/)
    assert.match((result.content as string), /小任务/)
    assert.match((result.content as string), /fetch reddit list/)
    assert.match((result.content as string), /journal write/)
    assert.match((result.content as string), /真的卡住|真的无聊/)
    assert.match((result.content as string), /想做什么/)
    assert.match((result.content as string), /现在缺什么/)
    assert.match((result.content as string), /建议加什么/)
    assert.match((result.content as string), /不要频繁/)
