...
const RESERVED_TOP_LEVEL = new Set(['browser', 'data', 'db-backups', 'memory', 'notebook', 'skill-drafts'])
...
export function createWorkspaceFileTool(deps: WorkspaceFileToolDeps = {}): Tool<Args> {
  ...
  return {
    ...
    description: [
      ...
      '适合 notes、drafts、creative 等自建目录; notebook/memory/skill-drafts/browser 等 managed path 必须走专用工具.',
